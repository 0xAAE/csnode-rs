...
use csp2p_rs::{CSHost, NodeId, NodeInfo, RawPacket};
...
impl Network {
    pub fn new(conf: SharedConfig, blocks: SharedBlocks) -> Box<Network> {
        ...
        Box::new(Network {
            ...
            collect_thread: start_collect(
                conf.clone(),
                stop_flag_instance.clone(),
                rx_raw,
                tx_cmd,
                tx_msg,
            ),
    ...
    pub fn stop(mut self) {
        ...
        self.collect_thread
            .join()
            .expect("Failed to stop packet collector");
        self.neighbours_thread
            .join()
            .expect("Failed to stop neihbourhood");
        self.processor_thread
            .join()
            .expect("Failed to stop message processor");
        self.sender_thread
            .join()
            .expect("Failed to stop fragment sender");
    }
}
...
fn start_neighbourhood(
    stop_flag: Arc<AtomicBool>,
    mut neighbourhood: CommandProcessor,
) -> JoinHandle<()> {
...
fn start_msg_processor(
    stop_flag: Arc<AtomicBool>,
    mut msg_processor: MessageProcessor,
) -> JoinHandle<()> {
    ...
    if !hosts_filename.is_empty() {
        match File::open(PathBuf::from(&hosts_filename)) {
            Err(e) => {
                ...
                known_hosts.push(NodeInfo {
                    id: "HBxj19cnpayn46GSqBGyKQXMaLThH4quuPt5gf8aFndg"
                        .from_base58()
                        .unwrap(),
