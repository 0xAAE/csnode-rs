...
fn test_try_parse_level() {
    assert_eq!(
        Data::try_parse_level("%Severity% >= debug"),
        Some(LevelFilter::Debug)
    );
    assert_eq!(
        Data::try_parse_level("%Severity% >= trace"),
        Some(LevelFilter::Trace)
    );
    assert_eq!(
        Data::try_parse_level("%Severity% >= info"),
        Some(LevelFilter::Info)
    );
    assert_eq!(
        Data::try_parse_level("%Severity% >= warning"),
        Some(LevelFilter::Warn)
    );
    assert_eq!(
        Data::try_parse_level("%Severity% >= error"),
