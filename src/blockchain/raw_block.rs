...
static OFFSET_SEQUENCE: usize = size_of::<u8>() + // block version
...
impl RawBlock {
    ...
    pub fn new_from_stream(bytes: Vec<u8>) -> Option<(RawBlock, Vec<u8>)> {
        ...
        match validate_raw_block(&bytes[block_begin..]) {
            ...
            Some(pos) => {
                ...
                Some((
                    RawBlock {
                        data: block_data[block_begin..].to_vec(),
                    },
                    next_data.to_vec(),
                ))
            }
    ...
    pub fn sequence(&self) -> Option<u64> {
        ...
        if self.data.len() >= (pos + size_of::<u64>()) {
            return Some(deserialize_from(&self.data[pos..]).unwrap());
        }
        None
    }
}
...
pub fn validate_raw_block(bytes: &[u8]) -> Option<usize> {
    ...
    let mut pos = size_of::<u8>() +   // version
        1 + HASH_SIZE +     // prev hash (1b size + 32b)
    ...
    let consensus_bits: u64 =
        deserialize_from(&bytes[pos + size_of_val(&consensus_cnt)..]).unwrap();
    let sig_blk_cnt = consensus_bits.count_ones() as usize;
    pos += size_of_val(&consensus_cnt)
        + size_of_val(&consensus_bits)
        + (consensus_cnt as usize * PUBLIC_KEY_SIZE);
    ...
    for _ in 0..contr_cnt {
        ...
        pos +=
            size_of_val(&item_sig_cnt) + (size_of::<u8>() + SIGNATURE_SIZE) * item_sig_cnt as usize;
    }
